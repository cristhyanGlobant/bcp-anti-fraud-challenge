...
public class AntiFraudServiceTests
{
    ...
    public async Task ValidateTransaction_ShouldCallEvaluatorAndReturnStatus()
    {
        ...
        mockEvaluator
            .Setup(x => x.EvaluateAsync(It.IsAny<TransactionEvaluationInput>()))
        ...
        var result = await service.ValidateTransaction(evt);

        result.Should().Be(TransactionStatus.Approved);
        mockEvaluator.Verify(x => x.EvaluateAsync(It.IsAny<TransactionEvaluationInput>()), Times.Once);
    }
}
